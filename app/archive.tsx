    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => router.replace('/inbox' as any)} style={styles.backBtn}>
          <Feather name="x" size={28} color="#FF6B00" />
        </TouchableOpacity>
        <Text style={styles.title}>Archive Chats</Text>
        <View style={{ width: 40 }} />
      </View>
            ...
            renderItem={({ item }) => (
              ...
              >
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.name}>{item.otherUser && (item.otherUser.displayName || item.otherUser.name) ? (item.otherUser.displayName || item.otherUser.name) : 'Unknown User'}</Text>
